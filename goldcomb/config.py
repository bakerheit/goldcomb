...
Config lives at ``$GOLDCOMB_CONFIG_DIR`` or ``~/.config/goldcomb/config.json`` and is
...
def config_dir() -> Path:
    ...
    new = base / "goldcomb"
    legacy = base / "nexais"
    # One-time adoption of a pre-rename install: copy (not move) so nothing
    # is destroyed if the user runs an old build again.
